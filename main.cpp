...
void dumpbytes(const vector<char>& v, int bytesToRead)
{
...
int main(int, char **)
{
	...
	GLFWwindow *window = glfwCreateWindow(1280, 720, "fuck off", NULL, NULL);
	if (window == NULL)
		return 1;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ImGui::Begin("Triangle pos/colour");	// label of the window
		static float rotation = 0.0;				// init value
		ImGui::SliderFloat("rotation", &rotation, 0, 2 * PI);	// name, initial value and minimum and max value
		...
		if(ImGui::BeginMenuBar()) {
			if (ImGui::BeginMenu("File")) {
				if (ImGui::MenuItem("Open..", "Ctrl+O")) { 
					// inFile.open("C:\\Users\\TTGCh\\Desktop\\dw6-saves\\save.dat");

					
					inFile.open("C:\\Users\\TTGCh\\Desktop\\dw6-saves\\save.dat", ios::in | ios::binary);
					if (inFile.is_open()) {
						cout << "File open" << endl;
						// start pos is at beginning
						streampos start = inFile.tellg();

						// seek to end of file
						inFile.seekg(0, std::ios::end);

						// now at the end, set this as the end pos
						...
						dumpbytes(contents, 1);
						//hexdump(contents.data(), 4);
						printf("%d", off[0].xp);
					}

					// if (!inFile) {
        			// 	// An error occurred!
        			// 	inFile.clear(); // so it is usable again.
    				// }
					// assumes online one number per line
		...
		triangle_shader.setUniform("color", 1.0f, 1.0f, 1.0f); // just set color to rainbow
